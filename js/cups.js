var CUPS={
    ...
    0:{code:"UNK",name:"Unknown",BCDp:[25,50,75],Kc:[0,0,0],plant:[1,1],harvest:[12,31]},
    99:{code:"SOIL",name:"Soil",BCDp:[25,50,75],Kc:[0,0,0],plant:[1,1],harvest:[12,31]}
    },
    doy_exp:function(band,p_h,year) {
	var band = typeof band !== 'undefined' ?  band : 'b(0)';
	var year = typeof year !== 'undefined' ?  year : 2015;
	var key;
	var doy;
	var data;
	var exp='';
	...
	for (key in dwr) {
	    data=dwr[key];
	    doy=ee.Date.from.YMD(year,data[p_h][0],data[p_h][1]).getRelative('day','year');
	    exp+="("+band+"=="+key+")?"+dwr[key]+":";
	}
	exp+="0";
	return exp;
    },
    plant_exp:function(band,year) { return this.doy_exp(band,'plant',year);},
    harvest_exp:function(band,year) { return this.doy_exp(band,'harvest',year);},
}
